...
<h1>NanoClaw: 基于Go语言的超高效 AI 助手</h1>

<h3>10$硬件 · 10MB内存 · 1秒启动 · 皮皮虾，我们走！</h3>

  <p>
    <br>
    <a href="https://nanoclaw.io">官方网站</a>
...
🦐 **NanoClaw** 是一个受 [nanobot](https://github.com/HKUDS/nanobot) 启发的超轻量级个人 AI 助手。它采用 **Go 语言** 从零重构，经历了一个“自举”过程——即由 AI Agent 自身驱动了整个架构迁移和代码优化。
...
>
> - **无加密货币 (NO CRYPTO):** NanoClaw **没有** 发行任何官方代币、Token 或虚拟货币。所有在 `pump.fun` 或其他交易平台上的相关声称均为 **诈骗**。
> - **官方域名:** 唯一的官方网站是 **[nanoclaw.io](https://nanoclaw.io)**，公司官网是 **[probnotas.com](https://probnotas.com)**。
> - **警惕:** 许多 `.ai/.org/.com/.net/...` 后缀教域名被第三方抢注，请勿轻信。
> - **注意:** nanoclaw正在初期的快速功能开发阶段，可能有尚未修复的网络安全问题，在1.0正式版发布前，请不要将其部署到生产环境中
> - **注意:** nanoclaw最近合并了大量PRs，近期版本可能内存占用较大(10~20MB)，我们将在功能较为收敛后进行资源占用优化.

## 📢 新闻 (News)

2026-02-16 🎉 NanoClaw 在一周内突破了12K star! 感谢大家的关注！NanoClaw 的成长速度超乎我们预期. 由于PR数量的快速膨胀，我们亟需社区开发者参与维护. 我们需要的志愿者角色和roadmap已经发布到了[这里](docs/ROADMAP.md), 期待你的参与！

2026-02-13 🎉 **NanoClaw 在 4 天内突破 5000 Stars！** 感谢社区的支持！由于正值中国春节假期，PR 和 Issue 涌入较多，我们正在利用这段时间敲定 **项目路线图 (Roadmap)** 并组建 **开发者群组**，以便加速 NanoClaw 的开发。
🚀 **行动号召：** 请在 GitHub Discussions 中提交您的功能请求 (Feature Requests)。我们将在接下来的周会上进行审查和优先级排序。

2026-02-09 🎉 **NanoClaw 正式发布！** 仅用 1 天构建，旨在将 AI Agent 带入 10 美元硬件与 <10MB 内存的世界。🦐 NanoClaw（皮皮虾），我们走！
...
| 🧩 全栈工程师模式 | 🗂️ 日志与规划管理 | 🔎 网络搜索与学习 |
| ----------------- | ----------------- | ----------------- |
| 开发 • 部署 • 扩展 | 日程 • 自动化 • 记忆 | 发现 • 洞察 • 趋势 |

### 📱 在手机上轻松运行

nanoclaw 可以将你10年前的老旧手机废物利用，变身成为你的AI助理！快速指南:
...
然后跟随下面的“快速开始”章节继续配置nanoclaw即可使用！

### 🐜 创新的低占用部署

NanoClaw 几乎可以部署在任何 Linux 设备上！
...
从 [Release 页面](https://github.com/probnotas/nanoClaw/releases) 下载适用于您平台的固件。
...
您也可以使用 Docker Compose 运行 NanoClaw，无需在本地安装任何环境。
...
> [!TIP]
> **Docker 用户**: 默认情况下, Gateway 监听 `127.0.0.1`，该端口不会暴露到容器外。如果需要通过端口映射访问健康检查接口，请在环境变量中设置 `NANOCLAW_GATEWAY_HOST=0.0.0.0` 或修改 `config.json`。
...
```bash
# 提问
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent -m "2+2 等于几？"
...
> [!TIP]
> 在 `~/.nanoclaw/config.json` 中设置您的 API Key。
...
**2. 配置 (Configure)** (`~/.nanoclaw/config.json`)
...
> `request_timeout` 为可选项，单位为秒。若省略或设置为 `<= 0`，NanoClaw 使用默认超时（120 秒）。
...
NanoClaw 支持多种聊天平台，使您的 Agent 能够连接到任何地方。
...
只需通过 CLI 或任何集成的聊天应用发送一条消息，即可将 NanoClaw 连接到 Agent 社交网络。

\*\*阅读 `https://clawdchat.ai/skill.md` 并按照说明加入 [ClawdChat.ai](https://clawdchat.ai)

## ⚙️ 配置详解

配置文件路径: `~/.nanoclaw/config.json`

### 环境变量

你可以使用环境变量覆盖默认路径。这对于便携安装、容器化部署或将 nanoclaw 作为系统服务运行非常有用。这些变量是独立的，控制不同的路径。

| 变量              | 描述                                                                                                                             | 默认路径                  |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `NANOCLAW_CONFIG` | 覆盖配置文件的路径。这直接告诉 nanoclaw 加载哪个 `config.json`，忽略所有其他位置。 | `~/.nanoclaw/config.json` |
| `NANOCLAW_HOME`   | 覆盖 nanoclaw 数据根目录。这会更改 `workspace` 和其他数据目录的默认位置。          | `~/.nanoclaw`             |

**示例：**

```bash
# 使用特定的配置文件运行 nanoclaw
# 工作区路径将从该配置文件中读取
NANOCLAW_CONFIG=/etc/nanoclaw/production.json nanoclaw gateway

# 在 /opt/nanoclaw 中存储所有数据运行 nanoclaw
# 配置将从默认的 ~/.nanoclaw/config.json 加载
# 工作区将在 /opt/nanoclaw/workspace 创建
...
NanoClaw 将数据存储在您配置的工作区中（默认：`~/.nanoclaw/workspace`）：
...
NanoClaw 可以自动执行周期性任务。在工作区创建 `HEARTBEAT.md` 文件：
...
- `NANOCLAW_HEARTBEAT_ENABLED=false` 禁用
- `NANOCLAW_HEARTBEAT_INTERVAL=60` 更改间隔
...
> **新功能！** NanoClaw 现在采用**以模型为中心**的配置方式。只需使用 `厂商/模型` 格式（如 `zhipu/glm-4.7`）即可添加新的 provider——**无需修改任何代码！**
...
> 运行 `nanoclaw auth login --provider anthropic` 来设置 OAuth 凭证。
...
为同一个模型名称配置多个端点——NanoClaw 会自动在它们之间轮询：
...
| 命令                      | 描述               |
| ------------------------- | ------------------ |
| `nanoclaw onboard`        | 初始化配置和工作区 |
| `nanoclaw agent -m "..."` | 与 Agent 对话      |
| `nanoclaw agent`          | 交互式聊天模式     |
| `nanoclaw gateway`        | 启动网关 (Gateway) |
| `nanoclaw status`         | 显示状态           |
| `nanoclaw cron list`      | 列出所有定时任务   |
| `nanoclaw cron add ...`   | 添加定时任务       |

### 定时任务 / 提醒 (Scheduled Tasks)

NanoClaw 通过 `cron` 工具支持定时提醒和重复任务：
...
任务存储在 `~/.nanoclaw/workspace/cron/` 中并自动处理。
