...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.nanoclaw/workspace" {
		t.Errorf("expected workspace '~/.nanoclaw/workspace', got '%s'", workspace)
	}
...
func TestConvertToNanoClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToNanoClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.nanoclaw/workspace" {
		t.Errorf("expected workspace '~/.nanoclaw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToNanoClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToNanoClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.nanoclaw/workspace" {
		t.Errorf("expected workspace '~/.nanoclaw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
