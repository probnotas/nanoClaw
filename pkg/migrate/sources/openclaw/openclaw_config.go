...
func (c *OpenClawConfig) ConvertToNanoClaw(sourceHome string) (*NanoClawConfig, []string, error) {
	cfg := &NanoClawConfig{}
	var warnings []string
	...
	if c.HasSkills() {
		warnings = append(
			warnings,
			fmt.Sprintf(
				"Skills (%d entries) not automatically migrated - reinstall via nanoclaw CLI",
				len(c.Skills.Entries),
			),
		)
	}
	if c.HasMemory() {
		warnings = append(warnings, "Memory backend config not migrated - NanoClaw uses SQLite with vector embeddings")
	}
	if c.HasCron() {
		warnings = append(
			warnings,
			"Cron job scheduling not supported in NanoClaw - consider using external schedulers",
		)
	}
	if c.HasHooks() {
		warnings = append(warnings, "Webhook hooks not supported in NanoClaw - use event system instead")
	}
	if c.HasSession() {
		warnings = append(warnings, "Session scope config differs - NanoClaw uses per-agent sessions by default")
	}
...
func (c *OpenClawConfig) convertChannels(warnings *[]string) ChannelsConfig {
	...
	if c.Channels.Signal != nil {
		*warnings = append(*warnings, "Channel 'signal': No NanoClaw adapter available")
	}
	if c.Channels.Matrix != nil {
		*warnings = append(*warnings, "Channel 'matrix': No NanoClaw adapter available")
	}
	if c.Channels.IRC != nil {
		*warnings = append(*warnings, "Channel 'irc': No NanoClaw adapter available")
	}
	if c.Channels.Mattermost != nil {
		*warnings = append(*warnings, "Channel 'mattermost': No NanoClaw adapter available")
	}
	...
	if c.Channels.BlueBubbles != nil {
		*warnings = append(
			*warnings,
			"Channel 'bluebubbles': No NanoClaw adapter available - consider iMessage instead",
		)
	}
...
func (c *NanoClawConfig) ToStandardConfig() *config.Config {
