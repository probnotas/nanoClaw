...
func TestDefaultConfig_WorkspacePath_Default(t *testing.T) {
	...
	t.Setenv("NANOCLAW_HOME", "")
	// Set a known home for consistent test results
	t.Setenv("HOME", "/tmp/home")

	cfg := DefaultConfig()
	want := filepath.Join("/tmp/home", ".nanoclaw", "workspace")
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	t.Setenv("NANOCLAW_HOME", "/custom/nanoclaw/home")

	cfg := DefaultConfig()
	want := "/custom/nanoclaw/home/workspace"

	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf("Workspace path with NANOCLAW_HOME = %q, want %q", cfg.Agents.Defaults.Workspace, want)
	}
}
