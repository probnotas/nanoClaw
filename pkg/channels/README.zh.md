# NanoClaw Channel System：完整开发指南

> **影响范围**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/nanoclaw/internal/gateway/`
...
import (
    "github.com/probnotas/nanoClaw/pkg/bus"
    "github.com/probnotas/nanoClaw/pkg/channels"     // 引用父包
    "github.com/probnotas/nanoClaw/pkg/config"
    "github.com/probnotas/nanoClaw/pkg/identity"      // 新增
    "github.com/probnotas/nanoClaw/pkg/media"          // 新增（如需媒体）
)
```
...
import (
    _ "github.com/probnotas/nanoClaw/pkg/channels/telegram"   // 触发 init() 注册
    _ "github.com/probnotas/nanoClaw/pkg/channels/discord"
    _ "github.com/probnotas/nanoClaw/pkg/channels/your_new_channel"  // 新增
)
```
...
4. **Pico Protocol**：`pkg/channels/nano/` 实现了一个自定义的 NanoClaw 原生协议 channel，通过 WebSocket webhook (`/pico/ws`) 接收消息。
...
8. **ReasoningChannelID**：大多数 channel config 都包含 `reasoning_channel_id` 字段，用于将 LLM 的思维链（reasoning/thinking）路由到指定 channel（WhatsApp、Telegram、Feishu、Discord、MaixCam、QQ、DingTalk、Slack、LINE、OneBot、WeCom、WeComApp）。注意：`NanoConfig` 目前不包含该字段。`BaseChannel` 通过 `WithReasoningChannelID` 选项和 `ReasoningChannelID()` 方法暴露此配置。