# NanoClaw Channel System: Complete Development Guide

> **Scope**: `pkg/channels/`, `pkg/bus/`, `pkg/media/`, `pkg/identity/`, `cmd/nanoclaw/internal/gateway/`
...
import (
    "github.com/probnotas/nanoClaw/pkg/bus"
    "github.com/probnotas/nanoClaw/pkg/channels"     // Reference parent package
    "github.com/probnotas/nanoClaw/pkg/config"
    "github.com/probnotas/nanoClaw/pkg/identity"      // New
    "github.com/probnotas/nanoClaw/pkg/media"          // New (if media support needed)
)
```
...
```go
// cmd/nanoclaw/internal/gateway/helpers.go
import (
    _ "github.com/probnotas/nanoClaw/pkg/channels/telegram"   // Triggers init() registration
    _ "github.com/probnotas/nanoClaw/pkg/channels/discord"
    _ "github.com/probnotas/nanoClaw/pkg/channels/your_new_channel"  // New addition
)
```
...
// cmd/nanoclaw/internal/gateway/helpers.go
import (
    _ "github.com/probnotas/nanoClaw/pkg/channels/matrix"
)
```
...
| `pkg/channels/nano/` | `"nano"` | TypingCapable, PlaceholderCapable, MessageEditor, WebhookHandler |
...
4. **Pico Protocol**: `pkg/channels/nano/` implements a custom NanoClaw native protocol channel that receives messages via WebSocket webhook (`/pico/ws`).
...
8. **ReasoningChannelID**: Most channel configs include a `reasoning_channel_id` field to route LLM reasoning/thinking output to a designated channel (WhatsApp, Telegram, Feishu, Discord, MaixCam, QQ, DingTalk, Slack, LINE, OneBot, WeCom, WeComApp). Note: `NanoConfig` does not currently expose this field. `BaseChannel` exposes this via the `WithReasoningChannelID` option and `ReasoningChannelID()` method.