...
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	...
	clientID := decodeBase64(
		"WU9VUl9DTElFTlRfSURfWkVST19SRUFDVA==", // YOUR_CLIENT_ID_HERE
	)
	clientSecret := decodeBase64("WU9VUl9DTElFTlRfU0VDUkVUX1pIUA==") // YOUR_CLIENT_SECRET_HERE
	return OAuthProviderConfig{
...
func buildAuthorizeURL(cfg OAuthProviderConfig, pkce PKCECodes, state, redirectURI string) string {
	...
	if isGoogle {
		...
		if strings.Contains(strings.ToLower(cfg.Issuer), "auth.openai.com") {
			params.Set("originator", "nanoclaw")
		}
		if cfg.Originator != "" {
