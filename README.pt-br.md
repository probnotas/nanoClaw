...
<h1>NanoClaw: Assistente de IA Ultra-Eficiente em Go</h1>
...
🦐 **NanoClaw** é um assistente pessoal de IA ultra-leve inspirado no [nanobot](https://github.com/HKUDS/nanobot), reescrito do zero em **Go** por meio de um processo de "auto-inicialização" (self-bootstrapping) — onde o próprio agente de IA conduziu toda a migração de arquitetura e otimização de código.
...
>
> * **SEM CRIPTOMOEDAS:** O NanoClaw **NÃO** possui nenhum token/moeda oficial. Todas as alegações no `pump.fun` ou outras plataformas de negociação são **GOLPES**.
> * **DOMÍNIO OFICIAL:** O **ÚNICO** site oficial é o **[nanoclaw.io](https://nanoclaw.io)**, e o site da empresa é o **[probnotas.com](https://probnotas.com)**.
> * **Aviso:** Muitos domínios `.ai/.org/.com/.net/...` foram registrados por terceiros, não são nossos.
> * **Aviso:** O NanoClaw está em fase inicial de desenvolvimento e pode ter problemas de segurança de rede não resolvidos. Não implante em ambientes de produção antes da versão v1.0.
> * **Nota:** O NanoClaw recentemente fez merge de muitos PRs, o que pode resultar em maior consumo de memória (10-20MB) nas versões mais recentes. Planejamos priorizar a otimização de recursos assim que o conjunto de funcionalidades estiver estável.


## 📢 Novidades

2026-02-16 🎉 NanoClaw atingiu 12K stars em uma semana! Obrigado a todos pelo apoio! O NanoClaw está crescendo mais rápido do que jamais imaginamos. Dado o alto volume de PRs, precisamos urgentemente de maintainers da comunidade. Nossos papéis de voluntários e roadmap foram publicados oficialmente [aqui](docs/ROADMAP.md) — estamos ansiosos para ter você a bordo!

2026-02-13 🎉 NanoClaw atingiu 5000 stars em 4 dias! Obrigado à comunidade! Estamos finalizando o **Roadmap do Projeto** e configurando o **Grupo de Desenvolvedores** para acelerar o desenvolvimento do NanoClaw.
...
2026-02-09 🎉 NanoClaw lançado oficialmente! Construído em 1 dia para trazer Agentes de IA para hardware de $10 com <10MB de RAM. 🦐 NanoClaw, Partiu!
...
Dê uma segunda vida ao seu celular de dez anos atrás! Transforme-o em um assistente de IA inteligente com o NanoClaw. Início rápido:
...
O NanoClaw pode ser implantado em praticamente qualquer dispositivo Linux!
...
Baixe o binário para sua plataforma na página de [releases](https://github.com/probnotas/nanoClaw/releases).
...
Você tambêm pode rodar o NanoClaw usando Docker Compose sem instalar nada localmente.
...
> [!TIP]
> **Usuários Docker**: Por padrão, o Gateway ouve em `127.0.0.1`, o que não é acessível a partir do host. Se você precisar acessar os endpoints de integridade ou expor portas, defina `NANOCLAW_GATEWAY_HOST=0.0.0.0` em seu ambiente ou atualize o `config.json`.
...
```bash
# Fazer uma pergunta
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent -m "Quanto e 2+2?"
...
> [!TIP]
> Configure sua API key em `~/.nanoclaw/config.json`.
...
**2. Configurar** (`~/.nanoclaw/config.json`)
...
> `request_timeout` é opcional e usa segundos. Se omitido ou definido como `<= 0`, o NanoClaw usa o timeout padrão (120s).
...
Converse com seu NanoClaw via Telegram, Discord, DingTalk, LINE ou WeCom.
...
O NanoClaw suporta dois tipos de integração WeCom:
...
> **Nota**: O WeCom App (callbacks de webhook) é servido pelo Gateway compartilhado (padrão 127.0.0.1:18790). Em produção use um proxy reverso HTTPS para expor a porta do Gateway, ou atualize `NANOCLAW_GATEWAY_HOST` para `0.0.0.0` se necessário.

</details>

## <img src="assets/clawdchat-icon.png" width="24" height="24" alt="ClawdChat"> Junte-se a Rede Social de Agentes

Conecte o NanoClaw a Rede Social de Agentes simplesmente enviando uma única mensagem via CLI ou qualquer App de Chat integrado.

**Leia `https://clawdchat.ai/skill.md` e siga as instruções para entrar no [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuração Detalhada

Arquivo de configuração: `~/.nanoclaw/config.json`

### Variáveis de Ambiente

Você pode substituir os caminhos padrão usando variáveis de ambiente. Isso é útil para instalações portáteis, implantações em contêineres ou para executar o nanoclaw como um serviço do sistema. Essas variáveis são independentes e controlam caminhos diferentes.

| Variável          | Descrição                                                                                                                             | Caminho Padrão            |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `NANOCLAW_CONFIG` | Substitui o caminho para o arquivo de configuração. Isso informa diretamente ao nanoclaw qual `config.json` carregar, ignorando todos os outros locais. | `~/.nanoclaw/config.json` |
| `NANOCLAW_HOME`   | Substitui o diretório raiz dos dados do nanoclaw. Isso altera o local padrão do `workspace` e de outros diretórios de dados.          | `~/.nanoclaw`             |

**Exemplos:**

```bash
# Executar o nanoclaw usando um arquivo de configuração específico
...
# Executar o nanoclaw com todos os seus dados armazenados em /opt/nanoclaw
# A configuração será carregada do ~/.nanoclaw/config.json padrão
# O workspace será criado em /opt/nanoclaw/workspace
...
O NanoClaw armazena dados no workspace configurado (padrão: `~/.nanoclaw/workspace`):
...
O NanoClaw roda em um ambiente sandbox por padrão. O agente so pode acessar arquivos e executar comandos dentro do workspace configurado.
...
|-------|--------|-----------|
| `workspace` | `~/.nanoclaw/workspace` | Diretório de trabalho do agente |
...
O NanoClaw pode executar tarefas periódicas automaticamente. Crie um arquivo `HEARTBEAT.md` no seu workspace:
...
* `NANOCLAW_HEARTBEAT_ENABLED=false` para desabilitar
* `NANOCLAW_HEARTBEAT_INTERVAL=60` para alterar o intervalo
...
> **Novidade!** NanoClaw agora usa uma abordagem de configuração **centrada no modelo**. Basta especificar o formato `fornecedor/modelo` (ex: `zhipu/glm-4.7`) para adicionar novos provedores—**nenhuma alteração de código necessária!**
...
```
> Execute `nanoclaw auth login --provider anthropic` para configurar credenciais OAuth.
...
Configure vários endpoints para o mesmo nome de modelo—NanoClaw fará round-robin automaticamente entre eles:
...
| Comando | Descrição |
| --- | --- |
| `nanoclaw onboard` | Inicializar configuração & workspace |
| `nanoclaw agent -m "..."` | Conversar com o agente |
| `nanoclaw agent` | Modo de chat interativo |
| `nanoclaw gateway` | Iniciar o gateway (para bots de chat) |
| `nanoclaw status` | Mostrar status |
| `nanoclaw cron list` | Listar todas as tarefas agendadas |
| `nanoclaw cron add ...` | Adicionar uma tarefa agendada |

### Tarefas Agendadas / Lembretes

O NanoClaw suporta lembretes agendados e tarefas recorrentes por meio da ferramenta `cron`:
...
As tarefas são armazenadas em `~/.nanoclaw/workspace/cron/` e processadas automaticamente.
...
Isso é normal se você ainda não configurou uma API key de busca. O NanoClaw fornecerá links úteis para busca manual.
...
Adicione a key em `~/.nanoclaw/config.json` se usar o Brave:
...
Isso acontece quando outra instância do bot está em execução. Certifique-se de que apenas um `nanoclaw gateway` esteja rodando por vez.
