...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in NanoClaw.
...
10. [Creating a New Provider in NanoClaw](#creating-a-new-provider-in-nanoclaw)
...
const CLIENT_ID = decode(
  "WU9VUl9DTElFTlRfSURfWkVST19SRUFDVA=="
);
const CLIENT_SECRET = decode("WU9VUl9DTElFTlRfU0VDUkVUX1pIUA==");
```
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<NanoClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- NanoClaw codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.nanoclaw/auth.json`:
...
## Creating a New Provider in NanoClaw

NanoClaw providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/nanoclaw/cmd_auth.go`:
...
nanoclaw auth login --provider your-provider

# List models (for Antigravity)
nanoclaw auth models

# Start the gateway
nanoclaw gateway

# Run an agent with a specific model
nanoclaw agent -m "Hello" --model your-model
```
...
export NANOCLAW_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export NANOCLAW_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `nanoclaw auth login --provider antigravity`
...
- Check auth profile storage: `~/.nanoclaw/auth.json`
- Re-run `nanoclaw auth login --provider antigravity`
