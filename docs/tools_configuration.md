...
NanoClaw's tools configuration is located in the `tools` field of `config.json`.
...
By default, NanoClaw blocks the following dangerous commands:
...
All configuration options can be overridden via environment variables with the format `NANOCLAW_TOOLS_<SECTION>_<KEY>`:

For example:
- `NANOCLAW_TOOLS_WEB_BRAVE_ENABLED=true`
- `NANOCLAW_TOOLS_EXEC_ENABLE_DENY_PATTERNS=false`
- `NANOCLAW_TOOLS_CRON_EXEC_TIMEOUT_MINUTES=10`
