...
<h1>NanoClaw: Trợ lý AI Siêu Nhẹ viết bằng Go</h1>
...
🦐 **NanoClaw** là trợ lý AI cá nhân siêu nhẹ, lấy cảm hứng từ [nanobot](https://github.com/HKUDS/nanobot), được viết lại hoàn toàn bằng **Go** thông qua quá trình "tự khởi tạo" (self-bootstrapping) — nơi chính AI Agent đã tự dẫn dắt toàn bộ quá trình chuyển đổi kiến trúc và tối ưu hóa mã nguồn.
...
>
> * **KHÔNG CÓ CRYPTO:** NanoClaw **KHÔNG** có bất kỳ token/coin chính thức nào. Mọi thông tin trên `pump.fun` hoặc các sàn giao dịch khác đều là **LỪA ĐẢO**.
> * **DOMAIN CHÍNH THỨC:** Website chính thức **DUY NHẤT** là **[nanoclaw.io](https://nanoclaw.io)**, website công ty là **[probnotas.com](https://probnotas.com)**.
> * **Cảnh báo:** Nhiều tên miền `.ai/.org/.com/.net/...` đã bị bên thứ ba đăng ký, không phải của chúng tôi.
> * **Cảnh báo:** NanoClaw đang trong giai đoạn phát triển sớm và có thể còn các vấn đề bảo mật mạng chưa được giải quyết. Không nên triển khai lên môi trường production trước phiên bản v1.0.
> * **Lưu ý:** NanoClaw gần đây đã merge nhiều PR, dẫn đến bộ nhớ sử dụng có thể lớn hơn (10–20MB) ở các phiên bản mới nhất. Chúng tôi sẽ ưu tiên tối ưu tài nguyên khi bộ tính năng đã ổn định.


## 📢 Tin tức

2026-02-16 🎉 NanoClaw đạt 12K stars chỉ trong một tuần! Cảm ơn tất cả mọi người! NanoClaw đang phát triển nhanh hơn chúng tôi tưởng tượng. Do số lượng PR tăng cao, chúng tôi cấp thiết cần maintainer từ cộng đồng. Các vai trò tình nguyện viên và roadmap đã được công bố [tại đây](docs/ROADMAP.md) — rất mong đón nhận sự tham gia của bạn!

2026-02-13 🎉 NanoClaw đạt 5000 stars trong 4 ngày! Cảm ơn cộng đồng! Chúng tôi đang hoàn thiện **Lộ trình dự án (Roadmap)** và thiết lập **Nhóm phát triển** để đẩy nhanh tốc độ phát triển NanoClaw.  
...
2026-02-09 🎉 NanoClaw chính thức ra mắt! Được xây dựng trong 1 ngày để mang AI Agent đến phần cứng $10 với RAM <10MB. 🦐 NanoClaw, Lên Đường!
...
NanoClaw có thể triển khai trên hầu hết mọi thiết bị Linux!
...
Tải file binary cho nền tảng của bạn từ [trang Release](https://github.com/probnotas/nanoClaw/releases).
...
Bạn cũng có thể chạy NanoClaw bằng Docker Compose mà không cần cài đặt gì trên máy.
...
> [!TIP]
> **Người dùng Docker**: Theo mặc định, Gateway lắng nghe trên `127.0.0.1`, không thể truy cập từ máy chủ. Nếu bạn cần truy cập các endpoint kiểm tra sức khỏe hoặc mở cổng, hãy đặt `NANOCLAW_GATEWAY_HOST=0.0.0.0` trong môi trường của bạn hoặc cập nhật `config.json`.
...
```bash
# Đặt câu hỏi
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent -m "2+2 bằng mấy?"
...
> [!TIP]
> Thiết lập API key trong `~/.nanoclaw/config.json`.
...
**2. Cấu hình** (`~/.nanoclaw/config.json`)
...
> `request_timeout` là tùy chọn và dùng đơn vị giây. Nếu bỏ qua hoặc đặt `<= 0`, NanoClaw sẽ dùng timeout mặc định (120s).
...
```bash
nanoclaw agent -m "Xin chào, bạn là ai?"
```
...
Trò chuyện với NanoClaw qua Telegram, Discord, DingTalk, LINE hoặc WeCom.
...
NanoClaw hỗ trợ hai loại tích hợp WeCom:
...
Kết nối NanoClaw với Mạng xã hội Agent chỉ bằng cách gửi một tin nhắn qua CLI hoặc bất kỳ ứng dụng Chat nào đã tích hợp.

**Đọc `https://clawdchat.ai/skill.md` và làm theo hướng dẫn để tham gia [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Cấu hình chi tiết

File cấu hình: `~/.nanoclaw/config.json`

### Biến môi trường

Bạn có thể ghi đè các đường dẫn mặc định bằng cách sử dụng các biến môi trường. Điều này hữu ích cho việc cài đặt di động, triển khai container hóa hoặc chạy nanoclaw như một dịch vụ hệ thống. Các biến này độc lập và kiểm soát các đường dẫn khác nhau.

| Biến              | Mô tả                                                                                                                             | Đường dẫn mặc định        |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `NANOCLAW_CONFIG` | Ghi đè đường dẫn đến file cấu hình. Điều này trực tiếp yêu cầu nanoclaw tải file `config.json` nào, bỏ qua tất cả các vị trí khác. | `~/.nanoclaw/config.json` |
| `NANOCLAW_HOME`   | Ghi đè thư mục gốc cho dữ liệu nanoclaw. Điều này thay đổi vị trí mặc định của `workspace` và các thư mục dữ liệu khác.          | `~/.nanoclaw`             |

**Ví dụ:**

```bash
# Chạy nanoclaw bằng một file cấu hình cụ thể
# Đường dẫn workspace sẽ được đọc từ trong file cấu hình đó
NANOCLAW_CONFIG=/etc/nanoclaw/production.json nanoclaw gateway

# Chạy nanoclaw với tất cả dữ liệu được lưu trữ trong /opt/nanoclaw
# Cấu hình sẽ được tải từ ~/.nanoclaw/config.json mặc định
# Workspace sẽ được tạo tại /opt/nanoclaw/workspace
...
NanoClaw lưu trữ dữ liệu trong workspace đã cấu hình (mặc định: `~/.nanoclaw/workspace`):
...
NanoClaw chạy trong môi trường sandbox theo mặc định. Agent chỉ có thể truy cập file và thực thi lệnh trong phạm vi workspace.
...
|----------|---------|-------|
| `workspace` | `~/.nanoclaw/workspace` | Thư mục làm việc của agent |
...
NanoClaw có thể tự động thực hiện các tác vụ định kỳ. Tạo file `HEARTBEAT.md` trong workspace:
...
* `NANOCLAW_HEARTBEAT_ENABLED=false` để tắt
* `NANOCLAW_HEARTBEAT_INTERVAL=60` để thay đổi khoảng thời gian
...
> **Tính năng mới!** NanoClaw hiện sử dụng phương pháp cấu hình **đặt mô hình vào trung tâm**. Chỉ cần chỉ định dạng `nhà cung cấp/mô hình` (ví dụ: `zhipu/glm-4.7`) để thêm nhà cung cấp mới—**không cần thay đổi mã!**
...
```
> Chạy `nanoclaw auth login --provider anthropic` để thiết lập thông tin xác thực OAuth.
...
Định cấu hình nhiều endpoint cho cùng một tên mô hình—NanoClaw sẽ tự động phân phối round-robin giữa chúng:
...
| Lệnh | Mô tả |
| --- | --- |
| `nanoclaw onboard` | Khởi tạo cấu hình & workspace |
| `nanoclaw agent -m "..."` | Trò chuyện với agent |
| `nanoclaw agent` | Chế độ chat tương tác |
| `nanoclaw gateway` | Khởi động gateway (cho bot chat) |
| `nanoclaw status` | Hiển thị trạng thái |
| `nanoclaw cron list` | Liệt kê tất cả tác vụ định kỳ |
| `nanoclaw cron add ...` | Thêm tác vụ định kỳ |

### Tác vụ định kỳ / Nhắc nhở

NanoClaw hỗ trợ nhắc nhở theo lịch và tác vụ lặp lại thông qua công cụ `cron`:
...
Các tác vụ được lưu trong `~/.nanoclaw/workspace/cron/` và được xử lý tự động.
...
Điều này là bình thường nếu bạn chưa cấu hình API key cho tìm kiếm. NanoClaw sẽ cung cấp các liên kết hữu ích để tìm kiếm thủ công.
...
Thêm key vào `~/.nanoclaw/config.json` nếu dùng Brave:
...
Điều này xảy ra khi có một instance bot khác đang chạy. Đảm bảo chỉ có một tiến trình `nanoclaw gateway` chạy tại một thời điểm.
