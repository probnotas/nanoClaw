<div align="center">
  <img src="assets/logo.jpg" alt="NanoClaw" width="512">

  <h1>NanoClaw : Assistant IA Ultra-Efficace en Go</h1>
  ...
  <p>
    ...
    <br>
    <a href="https://nanoclaw.io"><img src="https://img.shields.io/badge/Website-nanoclaw.io-blue?style=flat&logo=google-chrome&logoColor=white" alt="Website"></a>
...
🦐 **NanoClaw** est un assistant personnel IA ultra-léger inspiré de [nanobot](https://github.com/HKUDS/nanobot), entièrement réécrit en **Go** via un processus d'auto-amorçage (self-bootstrapping) — où l'agent IA lui-même a piloté l'intégralité de la migration architecturale et de l'optimisation du code.
...
<table align="center">
  <tr align="center">
    <td align="center" valign="top">
      <p align="center">
        <img src="assets/nanoclaw_mem.gif" width="360" height="240">
      </p>
    </td>
    <td align="center" valign="top">
      <p align="center">
...
>
> * **PAS DE CRYPTO :** NanoClaw n'a **AUCUN** token/jeton officiel. Toute annonce sur `pump.fun` ou d'autres plateformes de trading est une **ARNAQUE**.
> * **DOMAINE OFFICIEL :** Le **SEUL** site officiel est **[nanoclaw.io](https://nanoclaw.io)**, et le site de l'entreprise est **[probnotas.com](https://probnotas.com)**.
> * **Attention :** De nombreux domaines `.ai/.org/.com/.net/...` sont enregistrés par des tiers et ne nous appartiennent pas.
> * **Attention :** NanoClaw est en phase de développement précoce et peut présenter des problèmes de sécurité réseau non résolus. Ne déployez pas en environnement de production avant la version v1.0.
> * **Note :** NanoClaw a récemment fusionné de nombreuses PR, ce qui peut entraîner une empreinte mémoire plus importante (10–20 Mo) dans les dernières versions. Nous prévoyons de prioriser l'optimisation des ressources dès que l'ensemble des fonctionnalités sera stabilisé.


## 📢 Actualités

2026-02-16 🎉 NanoClaw a atteint 12K étoiles en une semaine ! Merci à tous pour votre soutien ! NanoClaw grandit plus vite que nous ne l'avions jamais imaginé. Vu le volume élevé de PR, nous avons un besoin urgent de mainteneurs communautaires. Nos rôles de bénévoles et notre feuille de route sont officiellement publiés [ici](docs/ROADMAP.md) — nous avons hâte de vous accueillir !

2026-02-13 🎉 NanoClaw a atteint 5000 étoiles en 4 jours ! Merci à la communauté ! Nous finalisons la **Feuille de Route du Projet** et mettons en place le **Groupe de Développeurs** pour accélérer le développement de NanoClaw.
...
2026-02-09 🎉 NanoClaw est lancé ! Construit en 1 jour pour apporter les Agents IA au matériel à 10$ avec <10 Mo de RAM. 🦐 NanoClaw, c'est parti !
...
<img src="assets/compare.jpg" alt="NanoClaw" width="512">
...
<table align="center">
  ...
  <tr>
    <td align="center"><p align="center"><img src="assets/nanoclaw_code.gif" width="240" height="180"></p></td>
    <td align="center"><p align="center"><img src="assets/nanoclaw_memory.gif" width="240" height="180"></p></td>
    <td align="center"><p align="center"><img src="assets/nanoclaw_search.gif" width="240" height="180"></p></td>
  </tr>
  <tr>
...
Donnez une seconde vie à votre téléphone d'il y a dix ans ! Transformez-le en assistant IA intelligent avec NanoClaw. Démarrage rapide :
...
wget https://github.com/probnotas/nanoClaw/releases/download/v0.1.1/nanoclaw-linux-arm64
chmod +x nanoclaw-linux-arm64
pkg install proot
termux-chroot ./nanoclaw-linux-arm64 onboard
```

Puis suivez les instructions de la section « Démarrage Rapide » pour terminer la configuration !

<img src="assets/termux.jpg" alt="NanoClaw" width="512">

### 🐜 Déploiement Innovant à Faible Empreinte

NanoClaw peut être déployé sur pratiquement n'importe quel appareil Linux !
...
Téléchargez le binaire pour votre plateforme depuis la page des [releases](https://github.com/probnotas/nanoClaw/releases).
...
Vous pouvez également exécuter NanoClaw avec Docker Compose sans rien installer localement.
...
> [!TIP]
> **Utilisateurs Docker** : Par défaut, le Gateway écoute sur `127.0.0.1`, ce qui n'est pas accessible depuis l'hôte. Si vous avez besoin d'accéder aux endpoints de santé ou d'exposer des ports, définissez `NANOCLAW_GATEWAY_HOST=0.0.0.0` dans votre environnement ou mettez à jour `config.json`.

```bash
# 5. Voir les logs
docker compose -f docker/docker-compose.yml logs -f nanoclaw-gateway
...
```bash
# Poser une question
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent -m "Combien font 2+2 ?"

# Mode interactif
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent
```
...
> [!TIP]
> Configurez votre clé API dans `~/.nanoclaw/config.json`.
...
**2. Configurer** (`~/.nanoclaw/config.json`)
...
> `request_timeout` est optionnel et s'exprime en secondes. S'il est omis ou défini à `<= 0`, NanoClaw utilise le délai d'expiration par défaut (120s).
...
```bash
nanoclaw agent -m "Combien font 2+2 ?"
```
...
Discutez avec votre NanoClaw via Telegram, Discord, DingTalk, LINE ou WeCom
...
NanoClaw prend en charge deux types d'intégration WeCom :
...
Connectez NanoClaw au Réseau Social d'Agents simplement en envoyant un seul message via le CLI ou n'importe quelle application de chat intégrée.

**Lisez `https://clawdchat.ai/skill.md` et suivez les instructions pour rejoindre [ClawdChat.ai](https://clawdchat.ai)**

## ⚙️ Configuration

Fichier de configuration : `~/.nanoclaw/config.json`

### Variables d'Environnement

Vous pouvez remplacer les chemins par défaut à l'aide de variables d'environnement. Ceci est utile pour les installations portables, les déploiements conteneurisés ou l'exécution de nanoclaw en tant que service système. Ces variables sont indépendantes et contrôlent différents chemins.

| Variable          | Description                                                                                                                             | Chemin par Défaut         |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `NANOCLAW_CONFIG` | Remplace le chemin du fichier de configuration. Cela indique directement à nanoclaw quel `config.json` charger, en ignorant tous les autres emplacements. | `~/.nanoclaw/config.json` |
| `NANOCLAW_HOME`   | Remplace le répertoire racine des données nanoclaw. Cela modifie l'emplacement par défaut du `workspace` et des autres répertoires de données.          | `~/.nanoclaw`             |

**Exemples :**

```bash
# Exécuter nanoclaw en utilisant un fichier de configuration spécifique
# Le chemin du workspace sera lu à partir de ce fichier de configuration
NANOCLAW_CONFIG=/etc/nanoclaw/production.json nanoclaw gateway

# Exécuter nanoclaw avec toutes ses données stockées dans /opt/nanoclaw
# La configuration sera chargée à partir du fichier par défaut ~/.nanoclaw/config.json
# Le workspace sera créé dans /opt/nanoclaw/workspace
NANOCLAW_HOME=/opt/nanoclaw nanoclaw agent

# Utiliser les deux pour une configuration entièrement personnalisée
NANOCLAW_HOME=/srv/nanoclaw NANOCLAW_CONFIG=/srv/nanoclaw/main.json nanoclaw gateway
```

### Structure du Workspace

NanoClaw stocke les données dans votre workspace configuré (par défaut : `~/.nanoclaw/workspace`) :
...
NanoClaw s'exécute dans un environnement sandboxé par défaut. L'agent ne peut accéder aux fichiers et exécuter des commandes qu'au sein du workspace configuré.
...
|--------|------------|-------------|
| `workspace` | `~/.nanoclaw/workspace` | Répertoire de travail de l'agent |
...
```bash
export NANOCLAW_AGENTS_DEFAULTS_RESTRICT_TO_WORKSPACE=false
```
...
NanoClaw peut exécuter des tâches périodiques automatiquement. Créez un fichier `HEARTBEAT.md` dans votre workspace :
...
* `NANOCLAW_HEARTBEAT_ENABLED=false` pour désactiver
* `NANOCLAW_HEARTBEAT_INTERVAL=60` pour modifier l'intervalle
...
```bash
nanoclaw agent -m "Bonjour, comment ça va ?"
```
...
> **Nouveau !** NanoClaw utilise désormais une approche de configuration **centrée sur le modèle**. Spécifiez simplement le format `fournisseur/modèle` (par exemple, `zhipu/glm-4.7`) pour ajouter de nouveaux fournisseurs—**aucune modification de code requise !**
...
```
> Exécutez `nanoclaw auth login --provider anthropic` pour configurer les identifiants OAuth.
...
Configurez plusieurs points de terminaison pour le même nom de modèle—NanoClaw utilisera automatiquement le round-robin entre eux :
...
| ------------------------- | ------------------------------------- |
| `nanoclaw onboard`        | Initialiser la configuration & le workspace |
| `nanoclaw agent -m "..."` | Discuter avec l'agent                 |
| `nanoclaw agent`          | Mode de discussion interactif         |
| `nanoclaw gateway`        | Démarrer la passerelle                |
| `nanoclaw status`         | Afficher le statut                    |
| `nanoclaw cron list`      | Lister toutes les tâches planifiées   |
| `nanoclaw cron add ...`   | Ajouter une tâche planifiée           |

### Tâches Planifiées / Rappels

NanoClaw prend en charge les rappels planifiés et les tâches récurrentes via l'outil `cron` :
...
Les tâches sont stockées dans `~/.nanoclaw/workspace/cron/` et traitées automatiquement.
...
<img src="assets/wechat.png" alt="NanoClaw" width="512">

## 🐛 Dépannage

### La recherche web affiche « API 配置问题 »

C'est normal si vous n'avez pas encore configuré de clé API de recherche. NanoClaw fournira des liens utiles pour la recherche manuelle.
...
Ajoutez la clé dans `~/.nanoclaw/config.json` si vous utilisez Brave :
...
Cela se produit lorsqu'une autre instance du bot est en cours d'exécution. Assurez-vous qu'un seul `nanoclaw gateway` fonctionne à la fois.
