...
| 🧩 Full-Stack Engineer | 🗂️ Logging & Planning Management | 🔎 Web Search & Learning |
| ---------------------- | -------------------------------- | ------------------------ |
| Develop • Deploy • Scale | Schedule • Automate • Memory | Discovery • Insights • Trends |

### 📱 Run on old Android Phones

Give your decade-old phone a second life! Turn it into a smart AI Assistant with NanoClaw. Quick Start:
...
Download the firmware for your platform from the [release](https://github.com/probnotas/nanoClaw/releases) page.
...
**Raspberry Pi Zero 2 W:** Use the binary that matches your OS: 32-bit Raspberry Pi OS → `make build-linux-arm` (output: `build/nanoclaw-linux-arm`); 64-bit → `make build-linux-arm64` (output: `build/nanoclaw-linux-arm64`). Or run `make build-pi-zero` to build both.

## 🐳 Docker Compose

You can also run NanoClaw using Docker Compose without installing anything locally.
...
> [!TIP]
> **Docker Users**: By default, the Gateway listens on `127.0.0.1` which is not accessible from the host. If you need to access the health endpoints or expose ports, set `NANOCLAW_GATEWAY_HOST=0.0.0.0` in your environment or update `config.json`.
...
```bash
# Ask a question
docker compose -f docker/docker-compose.yml run --rm nanoclaw-agent -m "What is 2+2?"
...
> `request_timeout` is optional and uses seconds. If omitted or set to `<= 0`, NanoClaw uses the default timeout (120s).
...
Talk to your nanoclaw through Telegram, Discord, WhatsApp, DingTalk, LINE, or WeCom
...
NanoClaw can connect to WhatsApp in two ways:
...
If `session_store_path` is empty, the session is stored in `&lt;workspace&gt;/whatsapp/`. Run `nanoclaw gateway`; on first run, scan the QR code printed in the terminal with WhatsApp → Linked Devices.
...
NanoClaw supports two types of WeCom integration:
...
You can override default paths using environment variables. This is useful for portable installations, containerized deployments, or running nanoclaw as a system service. These variables are independent and control different paths.

| Variable          | Description                                                                                                                             | Default Path              |
|-------------------|-----------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| `NANOCLAW_CONFIG` | Overrides the path to the configuration file. This directly tells nanoclaw which `config.json` to load, ignoring all other locations. | `~/.nanoclaw/config.json` |
| `NANOCLAW_HOME`   | Overrides the root directory for nanoclaw data. This changes the default location of the `workspace` and other data directories.          | `~/.nanoclaw`             |

**Examples:**

```bash
# Run nanoclaw using a specific config file
...
# Run nanoclaw with all its data stored in /opt/nanoclaw
# Config will be loaded from the default ~/.nanoclaw/config.json
# Workspace will be created at /opt/nanoclaw/workspace
...
NanoClaw stores data in your configured workspace (default: `~/.nanoclaw/workspace`):
...
NanoClaw runs in a sandboxed environment by default. The agent can only access files and execute commands within the configured workspace.
...
| ----------------------- | ----------------------- | ----------------------------------------- |
| `workspace`             | `~/.nanoclaw/workspace` | Working directory for the agent           |
...
NanoClaw can perform periodic tasks automatically. Create a `HEARTBEAT.md` file in your workspace:
...
* `NANOCLAW_HEARTBEAT_ENABLED=false` to disable
* `NANOCLAW_HEARTBEAT_INTERVAL=60` to change interval
...
> **What's New?** NanoClaw now uses a **model-centric** configuration approach. Simply specify `vendor/model` format (e.g., `zhipu/glm-4.7`) to add new providers—**zero code changes required!**
...
> Run `nanoclaw auth login --provider anthropic` to paste your API token.
...
Configure multiple endpoints for the same model name—NanoClaw will automatically round-robin between them:
...
NanoClaw routes providers by protocol family:
...
| `nanoclaw cron list`      | List all scheduled jobs       |
| `nanoclaw cron add ...`   | Add a scheduled job           |

### Scheduled Tasks / Reminders

NanoClaw supports scheduled reminders and recurring tasks through the `cron` tool:

* **One-time reminders**: "Remind me in 10 minutes" → triggers once after 10min
* **Recurring tasks**: "Remind me every 2 hours" → triggers every 2 hours
* **Cron expressions**: "Remind me at 9am daily" → uses cron expression

Jobs are stored in `~/.nanoclaw/workspace/cron/` and processed automatically.

## 🤝 Contribute & Roadmap

PRs welcome! The codebase is intentionally small and readable. 🤗

See our full [Community Roadmap](https://github.com/probnotas/nanoClaw/blob/main/ROADMAP.md).
...
This is normal if you haven't configured a search API key yet. NanoClaw will provide helpful links for manual searching.
...
Add the key to `~/.nanoclaw/config.json` if using Brave:
...
This happens when another instance of the bot is running. Make sure only one `nanoclaw gateway` is running at a time.
