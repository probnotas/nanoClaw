...
func (s *appState) modelMenu() tview.Primitive {
	...
	items = append(items,
		...
		MenuItem{
			...
			Action: func() {
				s.addModel(
					nanoclawconfig.ModelConfig{ModelName: "new-model", Model: "openai/gpt-5.2"},
				)
				s.push(
...
func (s *appState) addModel(model nanoclawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []nanoclawconfig.ModelConfig) {
...
func isModelValid(model nanoclawconfig.ModelConfig) bool {
...
func (s *appState) testModel(model *nanoclawconfig.ModelConfig) {
	if model == nil {
		return
	}
