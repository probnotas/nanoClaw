...
import (
	...
	configstore "github.com/probnotas/nanoClaw/cmd/nanoclaw-launcher-tui/internal/config"
...
func Run() error {
	...
	if cfg == nil {
		cfg = nanoclawconfig.DefaultConfig()
	}
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open nanoclaw agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) startTalk() {
	...
	s.app.Suspend(func() {
		cmd := exec.Command("nanoclaw", "agent")
		cmd.Stdin = os.Stdin
...
func (s *appState) startGateway() {
	...
	cmd := exec.Command("nanoclaw", "gateway")
