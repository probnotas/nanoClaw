...
A standalone launcher for NanoClaw, providing visual JSON editing and OAuth provider authentication management.
...
# Run with default config path (~/.nanoclaw/config.json)
./nanoclaw-launcher
...
```
Usage: nanoclaw-config [options] [config.json]

Arguments:
  config.json    Path to the configuration file (default: ~/.nanoclaw/config.json)
...
After authentication, Google redirects to `GET /auth/callback`, which saves the credentials and redirects back to the nanoclaw-config UI.
...
Gets the running status of the `nanoclaw gateway` process.
...
Starts the `nanoclaw gateway` process in the background.
...
Stops the running `nanoclaw gateway` process.
