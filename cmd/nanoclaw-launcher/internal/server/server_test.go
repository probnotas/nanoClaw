...
func TestGetConfig_MissingFile_ReturnsDefault(t *testing.T) {
	mux := http.NewServeMux()
	RegisterConfigAPI(mux, "/tmp/nonexistent-nanoclaw-launcher-test/config.json")
...
func TestDefaultConfigPath(t *testing.T) {
	...
	if !strings.HasSuffix(path, filepath.Join(".nanoclaw", "config.json")) {
		t.Errorf("expected path ending with .nanoclaw/config.json, got %q", path)
	}
}
