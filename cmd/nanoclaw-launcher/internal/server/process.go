...
// RegisterProcessAPI registers endpoints to start, stop and check status of the nanoclaw gateway.
...
func handleStartGateway(w http.ResponseWriter, r *http.Request) {
	// Locate nanoclaw executable:
	// 1. Try same directory as current executable
	// 2. Fallback to just "nanoclaw" (relies on $PATH)
	execPath := "nanoclaw"

	if exe, err := os.Executable(); err == nil {
		dir := filepath.Dir(exe)
		candidate := filepath.Join(dir, "nanoclaw")
	...
	if err := cmd.Start(); err != nil {
		log.Printf("Failed to start nanoclaw gateway: %v\n", err)
		http.Error(w, fmt.Sprintf("Failed to start gateway: %v", err), http.StatusInternalServerError)
		return
	}
	...
	log.Printf("Started nanoclaw gateway (PID: %d) from %s\n", cmd.Process.Pid, execPath)
...
func handleStopGateway(w http.ResponseWriter, r *http.Request) {
	var err error
	if runtime.GOOS == "windows" {
		// Kill via taskkill finding nanoclaw.exe (though it might kill this config tool if it's named nanoclaw-launcher.exe...? No, /IM does exact match usually, but just to be safe let's stop exactly nanoclaw.exe)
		// Alternatively, we use powershell to kill processes with commandline containing 'gateway'
		psCmd := `Get-WmiObject Win32_Process | Where-Object { $_.CommandLine -match 'nanoclaw.*gateway' } | ForEach-Object { Stop-Process $_.ProcessId -Force }`
		err = exec.Command("powershell", "-Command", psCmd).Run()
	} else {
		// Linux/macOS
		err = exec.Command("pkill", "-f", "nanoclaw gateway").Run()
	}
	...
	log.Printf("Stopped nanoclaw gateway processes.\n")
	w.Header().Set("Content-Type", "application/json")
