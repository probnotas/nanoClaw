...
import (
	...
	"github.com/probnotas/nanoClaw/pkg/auth"
...
func authLoginGoogleAntigravity() error {
	...
	fmt.Println("Try it: nanoclaw agent -m \"Hello world\"")
...
func authStatusCmd() error {
	...
	if len(store.Credentials) == 0 {
		...
		fmt.Println("Run: nanoclaw auth login --provider <name>")
		return nil
	}
...
func authModelsCmd() error {
	...
	if err != nil || cred == nil {
		return fmt.Errorf(
			"not logged in to Google Antigravity.\nrun: nanoclaw auth login --provider google-antigravity",
		)
	}
