...
import (
	...
	"github.com/probnotas/nanoClaw/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to nanoclaw",
		Args:  cobra.NoArgs,
		Example: `  nanoclaw migrate
  nanoclaw migrate --from openclaw
  nanoclaw migrate --dry-run
  nanoclaw migrate --refresh
  nanoclaw migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.nanoclaw)")
